...
function collectState.update(dt, stateData)
  ...
  if not moveTo(pos, dt) then
    stateData.drops[ix] = nil
  end
  local ids = world.itemDropQuery(mcontroller.position(), 3)
  for i,id in pairs(ids) do
    if storageApi.isFull() then break end
    local item = world.takeItemDrop(id, entity.id())
    if item ~= nil then
      storageApi.storeItem(id, item.name, item.count, item.data)
    end
  end
